function [FinalPACData] = FinalPhaseAmplitudeCoupling(Folder,Channel)
...
for ind0 = 1:length(subfolders)
    ...
    signalsdsdt = signalsdsdt(end-60*Fs:end);
    ...
    FinalPACData(ind0).PhaseBins = phasebins;
    FinalPACData(ind0).AmplitudeBins = amplitudebins;
    FinalPACData(ind0).NumberOfSurrogates = NumberOfSurrogates;
    FinalPACData(ind0).MI = MI;
    FinalPACData(ind0).PhaseBinsinterp = pbi;
    FinalPACData(ind0).AmplitudeBinsinterp = abi;
    FinalPACData(ind0).MIinterp = MIinterp;
...
mkdir(PACFolder);
cd(PACFolder);
save(strcat('FinalPACData','-',(animal),'-',(channelname),'.mat'),'FinalPACData');
