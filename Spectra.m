...
function [SpectraStructure] = SpectraWYW(Folder,Channel)
...
%     MeanCoh(fF>=30 & fF<=120) = smoothdata(MeanCoh(fF>=30 & fF<=120),'gaussian',params.smoothfactorhigh);
    ...
    SpectraStructure(ind0).Session = session;
    SpectraStructure(ind0).Parameters = params;
    SpectraStructure(ind0).fF = fF;
    SpectraStructure(ind0).MeanSpectrum = MeanSpec;
    SpectraStructure(ind0).SpecgFull = SpecgFull;
    SpectraStructure(ind0).X = x;
    SpectraStructure(ind0).Y = y;
    SpectraStructure(ind0).Distance = distance;
    SpectraStructure(ind0).Speed = speed;
    SpectraStructure(ind0).DeltaPower = PowerD;
    SpectraStructure(ind0).ThetaPower = PowerT;
    SpectraStructure(ind0).AlphaPower = PowerA;
    SpectraStructure(ind0).BetaPower = PowerB;
    SpectraStructure(ind0).LowGammaPower = PowerLG;
    SpectraStructure(ind0).HighGammaPower = PowerHG;
    SpectraStructure(ind0).DeltaFrequency = FreqD;
    SpectraStructure(ind0).ThetaFrequency = FreqT;
    SpectraStructure(ind0).AlphaFrequency = FreqA;
    SpectraStructure(ind0).BetaFrequency = FreqB;
    SpectraStructure(ind0).LowGammaFrequency = FreqLG;
    SpectraStructure(ind0).HighGammaFrequency = FreqHG;
    
end

%Session Analysis
MeanSpec = cell2mat({SpectraStructure.MeanSpectrum}');
...
mkdir(Spectrums);
cd(Spectrums);
figure2name = strcat(animal,'-',Channel,'-','SpectrumFamiliarity');
...
mkdir(SpectraFolder);
cd(SpectraFolder);
save(strcat('Spectra','-',(animal),'-',Channel,'.mat'),'SpectraStructure');
